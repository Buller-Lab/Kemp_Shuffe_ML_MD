...
df = pd.read_csv("kemp_shuffle_variants_stability.csv", sep=";")

FIO_cutoff = 1.35

df["Positive"] = df["FIO_HG3.17"] >= FIO_cutoff
y_true = df["Positive"].astype(int).values
sol = df["Stability"].values
...
print(f"Optimal Stability Range: [{best_lower:.4f}, {best_upper:.4f}]")
...
ax.set(xticks=np.arange(2), yticks=np.arange(2),
       ...
       title=f'Confusion Matrix\nStability Range [{best_lower:.4f}, {best_upper:.4f}]\nMCC = {best_mcc:.4f}',
...
plt.title(f"Stability vs FIO_HG3.17\nGreen = Positive (FIO ≥ {FIO_cutoff}) | Red = Negative")
plt.legend()
plt.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig("scatter_stability_vs_fio.png", dpi=200)
plt.show()

print("Plots saved:")
print("- confusion_matrix_range.png")
print("- scatter_stability_vs_fio.png")