...
def process_trajectory(traj_path, variant, atom1, atom1_alt, atom_2, atom_3, chain):
    max_retries=2
    ...
    selected_atom_4 = topology.select(atom_3)
    atom_triplets = [[selected_atom_1[0],selected_atom_3[0],selected_atom_4[0]],[selected_atom_2[0],selected_atom_3[0],selected_atom_4[0]]]
    angles = mdt.compute_angles(traj,atom_triplets)
    df = pd.DataFrame(angles)
    df.columns  = ["alpha_atom1","alpha_atom1_alt"]
    df["Variant"]=variant
    df["Path"]=traj_path
    df["Chain"]=chain    
    df["alpha_atom1"]=df["alpha_atom1"]* 57.2958
    df["alpha_atom1_alt"]=df["alpha_atom1_alt"]* 57.2958    
    df["Angle"] = df[["alpha_atom1","alpha_atom1_alt"]].max(axis=1)
    df=df.iloc[: , 2:]
    return df

def measure_angle(path, pre, ext, variant, atom1, atom1_alt, atom2, atom3, chain, out, sep):
    max_workers = 30
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(process_trajectory, traj_path, variant, atom1, atom1_alt, atom2, atom3, chain) for traj_path in prefixed]
